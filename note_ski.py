...
def run():
    ...
    # 月の前半（1日0:00〜15日23:59）のみ動作させる判定
    if now.day > 15:
        print(f"🛑 現在{now.day}日（月の後半）のため、動作を停止します。")
        return
    ...
    if 14 <= hour < 17:
        # 14:00 - 16:59
        keywords = ["休職中", "不登校", "在宅ワーク", "自分らしく", "応援して", "読書", "学び", "習慣", "自己啓発", "スキルアップ"]
    elif 17 <= hour < 20:
        # 17:00 - 19:59
        keywords = ["帰り道", "夕暮れ", "孤独", "お一人様", "応援して", "読書", "学び", "習慣", "自己啓発", "スキルアップ"]
    else:
        # それ以外の時間帯
        keywords = [
            "日々のこと", "つぶやき", "独り言", "吐き出す", 
            "反省会", "自分を立て直す", "本当はどうしたい", "もう無理", 
            "自分を好きになりたい", "言語化", "言葉にできない", 
            "本音", "自分のペース", "自分と向き合う", "心理学"
        ]
